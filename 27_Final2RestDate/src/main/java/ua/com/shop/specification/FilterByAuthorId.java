...
public class FilterByAuthorId implements Specification<Shop> {

	private final ShopFilterByAuthorId filter;
	private final List<Predicate> predicates = new ArrayList<>();
	
	private final static Pattern PATTERN = Pattern.compile("^([0-9]{1,17}\\.[0-9]{1,2})|([0-9]{1,17}\\,[0-9]{1,2})|([0-9]{1,17})$");
	
	public FilterByAuthorId(ShopFilterByAuthorId filter, int id) {
		this.filter = filter;
		filter.setId(id);
	...
	private void filterByAuthor(Root<Shop> root, CriteriaQuery<?> query, CriteriaBuilder cb){
		if(!filter.getAuthorIds().isEmpty()){
			predicates.add(root.get("titleSh").in(filter.getAuthorIds()));
		}
	}
	...
	private void fetch(Root<Shop> root, CriteriaQuery<?> query){
		if(!query.getResultType().equals(Long.class)){
			...
			root.fetch("titleSh").fetch("book").fetch("author");
		}
	}
