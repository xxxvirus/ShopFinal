...
<script type="text/javascript">
		...
		app.filter('byEntity', function() {
			return function(input, search) {
				if (!search) return input;
				var array = [];
				for (var i = 0; i < input.length; i++) {
					if (input[i].nameOfC.toLowerCase().indexOf(search.toLowerCase()) !== -1) {
						array.push(input[i])
					}
				}
				return array;
             ...
             $scope.save = function(){
             	$http({
             		...
             		for(var i = 0; i < $scope.pageArr.length; i++){
             			if(result.data.id==$scope.pageArr[i].id){
             				$scope.pageArr.splice(i, 1);
             			}
             		}
             		$scope.pageArr.push(result.data);
					})
                 $scope.cancel();
             }
		...
		<div class="col-md-10">
			...
			<div class="row">
				<table class="table table-striped text-center">
					<tr>
						<th class="text-center" ng-click="sort('nameOfC')"
							style="cursor: pointer;">Category<span
							class="glyphicon sort-icon" ng-show="sortKey=='nameOfC'"
							ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
						<th class="text-center">Update</th>
						<th class="text-center">Delete</th>
					</tr>
					<tr
						dir-paginate="item in pageArr|orderBy:sortKey:reverse|byEntity:search|itemsPerPage:10">
						<td>{{item.nameOfC}}</td>
	...
	<div class="row" ng-show="currentView == 'form'">
		<form name='frm' class="form-horizontal" novalidate>
			<div class="form-group">
				...
				<div class="col-md-6">
					<input name="name" ng-model="currentItem.nameOfC" class="form-control" ng-required='true' type="text">
				</div>
			</div>
			<div class="form-group">
